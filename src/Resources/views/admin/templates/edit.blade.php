    ...
    <div class="container mt-4">
        ...
        <form action="{{ route('email-templates.admin.templates.update', $template->id) }}" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="text" name="identifier" id="identifier" class="form-control" value="{{ $template->identifier }}" disabled>
            ...
            @foreach($locales as $locale)
                ...
                <div class="card mb-3">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            <label for="translations[{{ $locale }}][subject]">{{ __('email-templates::messages.subject') }}</label>
                            <input type="text" name="translations[{{ $locale }}][subject]" id="translations[{{ $locale }}][subject]" class="form-control" value="{{ old('translations.' . $locale . '.subject', $translation->subject ?? '') }}" required>
                        </div>
                        <div class="form-group">
                            <label for="translations[{{ $locale }}][body]">{{ __('email-templates::messages.body') }}</label>
                            <textarea name="translations[{{ $locale }}][body]" id="translations[{{ $locale }}][body]" class="form-control rich-text-editor" rows="5" required>{{ old('translations.' . $locale . '.body', $translation->body ?? '') }}</textarea>
                            <small class="form-text text-muted">
                                {{ __('email-templates::messages.available_placeholders') }}: 
                                @foreach($availablePlaceholders as $placeholder)
                                    <code>{{ '{{' }}{{ $placeholder->name }}{{ '}}' }}</code>@if(!$loop->last), @endif
                                @endforeach
                            </small>
                        </div>
                    </div>
            ...
            <a href="{{ route('email-templates.admin.templates.index') }}" class="btn btn-secondary">{{ __('email-templates::messages.cancel') }}</a>
        </form>
    </div>
@endsection
